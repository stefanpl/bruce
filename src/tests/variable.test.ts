...
Promise<IVariableCheckResult>
{
  ...
  async function _checkJsonSchema () {
    var valid = ajv.validate(variableType.schema, variable);
    if ( ! valid) {
  ...
  async function _runAdditionalValidators() {
    ...
    let validationResults = await Promise.all(variableType.additionalValidators.map((validator) => validator.validatorFunction(variable)));
...
describe('dynamic variables', function() {
  ...
  it('a variable can be checked against a type', async function () {
    ...
    expect(checkResult.checkSuccessful, 'The variable check should succeed!').to.be.true;
  ...
  it('should validate against a given schema', async function () {
    ...
    let someVariable = 'Should definitely match';
    let checkResult = await checkVariableAgainstType(someVariable, testVariableTypeWithSchema);
    expect(checkResult.checkSuccessful, 'The variable check should succeed').to.be.true;
    expect(checkResult.errors).to.be.empty;
    
    let anotherVariable: string = 123 as unknown as string;
    checkResult = await checkVariableAgainstType(anotherVariable, testVariableTypeWithSchema); 
    expect(checkResult.checkSuccessful, 'The variable check should fail!').to.be.false;
    expect(checkResult.errors).to.be.an('array').with.lengthOf(1);
