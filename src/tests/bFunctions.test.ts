...
describe('bruce functions', function() {
  ...
  it('validate a schema again the json meta schema', async function () {
    ...
    const schemaVariable: bVariable<Object> = {
      ...
      name: 'JSON Schema',
      description: `A json schema declaration validated against the latest`
      + ` [validation meta schema](https://json-schema.org/specification.html#meta-schemas).`
    }

    assert(valueIsValidForVariable(validSchema, schemaVariable), 'Valid schema should validate against meta schema')
    assert( ! valueIsValidForVariable(invalidSchema, schemaVariable), 'Invalid schema should not validate against meta schema')
