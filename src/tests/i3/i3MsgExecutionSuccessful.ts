...
describe('i3MsgExecutionSuccessful', function() {
  
  it('checks for successful and unsuccessful executions', async function () {
    ...
    await Promise.all(testCases.map(async testCase => {
      ...
      assert.strictEqual(i3MsgExecutionSuccessful(result), testCase.expectedSuccessful, 
      `i3msg command '${testCase.command}' did no yield the expected result.`)
    }))
