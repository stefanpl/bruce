...
export async function tmuxCreateSession (sessionName: string, command?: string): Promise<void> {
  await expectSuccessfulExecution(`tmux new -d -s`, command ? [sessionName, command] : [sessionName])
}
...
export async function tmuxStartSession (session: TmuxSession) {
  ...
  async function _createWindows() {
    ...
    for(let windowIndex = 0 ; windowIndex < windowArguments.length ; windowIndex++ ) {
      const createArguments = windowIndex === 0 ? ['new-session', '-s', session.name] : ['new-window']
      const allArguments = createArguments.concat(baseArguments, windowArguments[windowIndex])
      await expectSuccessfulExecution('tmux', allArguments)
    }
  }
  
  async function _sendKeys() {
    await Promise.all((session.windows as Array<TmuxWindow>).map(async (window, index) => {
      if ( ! window.keysToBeSent) return;
      const args = ['send-keys', '-t', `${session.name}:${index+1}`].concat(window.keysToBeSent)
  ...
  function _argsForWindow(window: TmuxWindow): Array<string> {
    ...
    return args.concat(returnAsArray( window.command ? window.command : session.defaultCommand ))
  }
}