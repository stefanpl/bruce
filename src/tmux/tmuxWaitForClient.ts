...
*/
export async function tmuxWaitForClient (maxTimeoutInMilliseconds: number = 2000): Promise<void> {
  assert(maxTimeoutInMilliseconds > 0, `Given timeout must be > 0, but was ${maxTimeoutInMilliseconds}`)
  let startTime = Date.now()
  return new Promise(async function(resolve, reject) {
    _checkForClients()
    async function _checkForClients() {
      if (Date.now() - startTime > maxTimeoutInMilliseconds) {
        return reject(Error(`Could not find a tmux client within ${maxTimeoutInMilliseconds} ms.`))
      }
